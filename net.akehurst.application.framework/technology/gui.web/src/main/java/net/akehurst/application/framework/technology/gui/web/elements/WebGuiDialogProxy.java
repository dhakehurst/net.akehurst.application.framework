...
public class WebGuiDialogProxy implements InvocationHandler, IIdentifiableObject {

    public WebGuiDialogProxy(final String afId, final IGuiRequest guiRequest, final IGuiScene scene, final DialogIdentity dialogId, final String content) {
        this.afId = afId;
    ...
    IGuiDialog getHandler() {
        if (null == this.handler) {
            try {
                this.handler = this.af.createObject(WebGuiDialog.class, this.afId, this.guiRequest, this.scene, this.dialogId, this.content);
        ...
        if (Arrays.asList(IGuiDialog.class.getMethods()).contains(method)) {
            ...
            if (method.getName().startsWith("get")) {
                ...
                if (IGuiElement.class == returnType) {
                    return new WebGuiElement(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiContainer.class == returnType) {
                    return new WebGuiContainer(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiText.class == returnType) {
                    return new WebGuiText(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiChart.class == returnType) {
                    return new WebGuiChart(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiTable.class == returnType) {
                    return new WebGuiTable(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiEditor.class == returnType) {
                    return new WebGuiEditor(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiDiagram.class == returnType) {
                    return new WebGuiDiagram(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiGraphViewer.class == returnType) {
                    return new WebGuiGraph(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else if (IGuiGraphViewer.class == returnType) {
                    return new WebGuiGrid(this.guiRequest, this.scene, this.getHandler(), elementName);
                } else {
                    return null;
                }
            } else {
